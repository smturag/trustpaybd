...
@section('mrc_content')
    ...
    <div class="page-content">
        ...
        <div class="page-breadcrumb d-flex align-items-center mb-3">
            <div class="breadcrumb-title pe-3">Merchant Profile</div>
            <div class="ps-3 ms-auto">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        <li class="breadcrumb-item"><a href="{{ route('merchant_dashboard') }}"><i class="bx bx-home-alt"></i> Dashboard</a>
                        </li>
        ...
        <div class="row">
            <div class="col-lg-4">
                        ...
                        <div class="d-flex flex-column align-items-center">
                            <div class="profile-image-container">
                                <img src="{{ $profile_data->profile_pic ? url('storage/app/public/'. $profile_data->profile_pic) : asset('static/backend/images/avatars/avatar-1.png') }}"
                                    alt="Merchant Profile" class="profile-image">
                            ...
                            <div class="profile-info">
                                ...
                                <p class="mb-1"><i class="bx bx-store me-1"></i>{{ $profile_data->username }}</p>
                                <span class="profile-badge">
                                    <i class="bx bx-check-shield me-1"></i>Verified Merchant
                                </span>
                            ...
                            <div class="profile-actions">
                                <a href="{{ route('merchant_dashboard') }}" class="btn btn-outline-primary">
                                    <i class="bx bx-home-alt"></i> Dashboard
                                </a>
                                <a href="{{ route('merchant.developer-index') }}" class="btn btn-outline-info">
                                    <i class="bx bx-code-alt"></i> Developer
                                </a>
                                <a href="{{ route('merchantlogout') }}" class="btn btn-outline-danger">
                ...
                <div class="card profile-card mt-4">
                    <div class="card-body">
                        <h5 class="card-title">
                            <i class="bx bx-key me-1"></i>API Access
                        </h5>

                        @php
                            // Get merchant API key if available
                            $merchantToken = \App\Models\MerchantPvtPublicKey::where('merchant_id', $profile_data->id)->first();
                        @endphp
                        ...
                        @if($merchantToken)
                            <div class="mb-3">
                                ...
                                <div class="d-flex">
                                    <input type="text" class="form-control" id="api_key_input" value="{{ substr($merchantToken->api_key, 0, 8) }}..." readonly>
                                    <button id="copyApiKeyButton" class="btn btn-outline-primary ms-2">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="d-flex">
                                    <input type="text" class="form-control" id="secret_key_input" value="{{ substr($merchantToken->secret_key, 0, 8) }}..." readonly>
                                    <button id="copySecretKeyButton" class="btn btn-outline-primary ms-2">
                                        <i class="bx bx-copy"></i>
                                    </button>
                                </div>
                                <small class="text-muted">Keep this secret key secure</small>
                            </div>

                            <div class="text-center">
                                <a href="{{ route('merchant.developer-index') }}" class="btn btn-sm btn-outline-primary">
                                    <i class="bx bx-code-alt me-1"></i>Developer Portal
                                </a>
                            </div>
                        @else
                            <div class="text-center py-3">
                                <p class="mb-3">No API key generated yet</p>
                                <a href="{{ route('merchant.developer.api-key-generate') }}" class="btn btn-sm btn-primary">
                                    <i class="bx bx-plus-circle me-1"></i>Generate API Key
                                </a>
                            </div>
                        @endif
                    </div>
            ...
            <div class="col-lg-8">
                <div class="card profile-card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('merchantProfileUpdate') }}" method="POST" enctype="multipart/form-data" id="profile-form" class="mt-4">
                            @csrf
                            ...
                            <div class="row mb-4">
                                ...
                                <div class="col-md-6">
                                    <label for="username" class="form-label">
                                        <i class="bx bx-id-card text-primary me-1"></i>PIN
                                    </label>
                                    <div class="input-group">
                                        <span class="input-group-text"><i class="bx bx-id-card"></i></span>
                                        <input type="text" class="form-control" id="pin" value="{{ $profile_data->pincode }}" name="pincode" >
                                    </div>
                                    <small class="text-muted">Used for login and identification (cannot be changed)</small>
                                </div>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="card bg-light border-0">
                                        <div class="card-body py-3">
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <p class="mb-1"><strong>Merchant ID:</strong> {{ $profile_data->id }}</p>
                ...
                <div class="card profile-card password-section mt-4 fade-in" id="password_div" style="display: none">
                    <div class="card-body">
                        ...
                        <form class="needs-validation mt-4" action="{{ route('merchantChangePassword') }}" method="POST" id="password-form">
                            @csrf
                            <div class="row mb-4">
                                <div class="col-md-12">
    ...
    <div class="card-body">
        <h5 class="card-title">
            <i class="bx bx-transfer-alt me-1"></i> Merchant Fee And Commission
        </h5>

        @php
            // Get all operator fee & commission data for this merchant
            $feeAndCommission = \App\Models\OperatorFeeCommission::where('merchant_id', auth('merchant')->user()->id)->get();

            // Active operators
            $operators = \App\Models\MfsOperator::where('status', 1)->get();
        @endphp

        @foreach($operators as $operator)
            <h6 class="mt-3 mb-2">
                {{ $operator->name }}
                <small class="text-muted">({{ ucfirst($operator->type) }})</small>
            </h6>

            @foreach(['deposit','withdraw'] as $action)
                @php
                    $record = $feeAndCommission->firstWhere(function ($r) use ($operator, $action) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (copyApiKeyButton) {
            copyApiKeyButton.addEventListener('click', function() {
                ...
                const fullApiKey = "{{ $merchantToken->api_key ?? '' }}";

                // If the API key is masked with ellipsis, replace it with the full key
                if (apiKeyInput.value.includes('...')) {
                    apiKeyInput.value = fullApiKey;
                }

                copyToClipboard(fullApiKey, this, 'API Key copied to clipboard');
            });
        }

        // Copy Secret Key functionality
        const copySecretKeyButton = document.getElementById('copySecretKeyButton');
        if (copySecretKeyButton) {
            copySecretKeyButton.addEventListener('click', function() {
                const secretKeyInput = document.getElementById('secret_key_input');

                // Get the full secret key
                const fullSecretKey = "{{ $merchantToken->secret_key ?? '' }}";

                // If the secret key is masked with ellipsis, replace it with the full key
                if (secretKeyInput.value.includes('...')) {
                    secretKeyInput.value = fullSecretKey;
                }

                copyToClipboard(fullSecretKey, this, 'Secret Key copied to clipboard');
            });
        }
