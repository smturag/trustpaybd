...
<head>
    ...
    <meta http-equiv="refresh" content="{{ config('session.lifetime') * 60 }}; {{ route('customerlogin') }}" />

    <title>@if(!isset($pageTitle))
            {{ config('app.name', 'Laravel') }}
        @else
            {{ 'Customer '.$pageTitle.' - '.config('app.name') }}
        @endif</title>
...
<div class="wrapper">
    @include('customer-panel.customer_sidebar')
    @include('customer-panel.customer_head')
