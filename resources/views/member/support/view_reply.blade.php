...
@endif
 ...
 <h5 class="mb-4">{{ translate('reply_ticket') }}</h5>		
                            ...
                            @if($ticket_object->status == 1)
                                    ...
                                    @endif
                                    <a href="{{route('ticketClose', $ticket_object->ticket)}}" class="btn btn-danger pull-right make-close-support" style="height: 35px;">Click To Make Close</a>
                            <div class="panel-heading">  #{{$ticket_object->ticket}} - {{$ticket_object->subject}}   
                             </div>
                            

                                    <form class="row g-3" method="POST" action="{{route('ticketReplyStore', $ticket_object->ticket)}}" accept-charset="UTF-8" >
                                        {{csrf_field()}}
                                                ...
                                                @foreach($ticket_data as $data)
                                                    <div class="panel-body">
                                                        <fieldset class="col-md-12">
                                                            @if($data->type == 1)
                                                                <legend><span style="color: #0e76a8">{{Auth::user()->fullname}}</span> , <small>{{ \Carbon\Carbon::parse($data->updated_at)->format('F dS, Y - h:i A') }}</small></legend>
                                                            @else
                                                                <legend><span style="color: #0e76a8">{{app_config('AppTitle')}}</span> , <small>{{ \Carbon\Carbon::parse($data->updated_at)->format('F dS, Y - h:i A') }}</small></legend>
                                                            @endif
                                       ...
                                       <div class="box-footer">
										<div class="row">
										 ...
										 <div class="col-md-12">
										<button type="submit" class="btn btn-info btn-block pull-right">{{ translate('submit_now') }}</button>
										</div>
										</div>
									  </div>
                                    </form>
