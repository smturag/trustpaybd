    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Edit Permission</h1>
                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Update permission details</p>
                </div>
            </div>
        </div>
            ...
            <form action="{{ route('admin.permissions.update', $permission) }}" method="POST" class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white @error('description') border-red-500 @enderror">{{ old('description', $permission->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
