...
<div class="row">
    <div class="col-xl-12 mx-auto">
        <div class="header d-flex align-items-center mb-3 flex-column flex-sm-row">
            <h6 class="mb-2 mb-sm-0 text-uppercase text-center text-sm-start ps-sm-3">User Fees & Commissions</h6>
            <a class="ms-sm-auto btn btn-sm btn-primary mt-2 mt-sm-0" href="{{ route('userList') }}">
                <i class="bx bx-left-arrow me-1"></i> Back to User
            </a>
        </div>
        <hr/>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <h5 class="mb-4">User: {{ $user->fullname }}</h5>
                ...
                <form id="feesForm" action="{{ route('member.updateFees', $user->id) }}" method="POST">
                    @csrf
                    ...
                    @foreach($operators as $operator)
                        ...
                        @foreach(['deposit','withdraw'] as $action)
                            @php
                                $record = $user->user_rate->firstWhere(function($r) use($operator,$action){
