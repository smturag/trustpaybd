...
<aside id="sidebar" class="fixed left-0 top-0 h-screen w-64 bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 text-white transition-all duration-300 ease-in-out overflow-y-auto z-50 shadow-2xl transform -translate-x-full lg:translate-x-0">
    <!-- Sidebar Header -->
    ...
    <nav class="px-4 py-6 space-y-2">
        ...
        <!-- Crypto Payout -->
        @if(can('view-crypto-payouts') || can('view-merchant-payouts'))
        ...
        <!-- Balance Manager -->
        @if(can('view-balance-manager'))
        <a href="{{ route('balance_manager', 'all') }}" class="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-slate-700/50 transition-all duration-200 group {{ request()->routeIs('balance_manager') ? 'bg-gradient-to-r from-blue-600 to-cyan-600 shadow-lg shadow-blue-500/30' : '' }}">
        ...
        <!-- All Users -->
        @if(canAny(['view-merchants', 'view-customers', 'view-users']))
        <div x-data="{ open: false }" class="space-y-1">
            <button @click="open = !open" class="w-full flex items-center justify-between px-4 py-3 rounded-lg hover:bg-slate-700/50 transition-all duration-200 group">
                <div class="flex items-center space-x-3">
        ...
        <!-- Method -->
        @if(canAny(['view-payment-methods', 'view-withdraw-methods']))
        <div x-data="{ open: false }" class="space-y-1">
            <button @click="open = !open" class="w-full flex items-center justify-between px-4 py-3 rounded-lg hover:bg-slate-700/50 transition-all duration-200 group">
                <div class="flex items-center space-x-3">
        ...
        <a href="{{ route('admin.activity-logs.index') }}" class="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-slate-700/50 transition-all duration-200 group">
        ...
        <!-- Settings -->
        @if(canAny(['view-settings', 'view-roles', 'view-admins']))
        <div x-data="{ open: false }" class="space-y-1">
            <button @click="open = !open" class="w-full flex items-center justify-between px-4 py-3 rounded-lg hover:bg-slate-700/50 transition-all duration-200 group">
                <div class="flex items-center space-x-3">
            ...
            <div x-show="open" x-transition class="ml-6 space-y-1 border-l-2 border-slate-700 pl-4">
                ...
                @endif
                @if(can('view-currency'))
                <a href="{{ route('admin.currency.index') }}" class="block px-3 py-2 text-sm rounded hover:bg-slate-700/30 transition-colors {{ request()->routeIs('admin.currency.index') ? 'text-cyan-400' : 'text-gray-300' }}">Currency Management</a>
                @endif
                <a href="{{ route('admin.pricing.index') }}" class="block px-3 py-2 text-sm rounded hover:bg-slate-700/30 transition-colors {{ request()->routeIs('admin.pricing.*') ? 'text-cyan-400' : 'text-gray-300' }}">Pricing Plans</a>
                @if(can('view-roles'))
                <a href="{{ route('admin.roles.index') }}" class="block px-3 py-2 text-sm rounded hover:bg-slate-700/30 transition-colors {{ request()->routeIs('admin.roles.*') || request()->routeIs('admin.permissions.*') ? 'text-cyan-400' : 'text-gray-300' }}">Role & Permissions</a>
                @endif
                @if(can('view-admins'))
                <a href="{{ route('admin.admin-management.index') }}" class="block px-3 py-2 text-sm rounded hover:bg-slate-700/30 transition-colors {{ request()->routeIs('admin.admin-management.*') ? 'text-cyan-400' : 'text-gray-300' }}">Admin Management</a>
                @endif
