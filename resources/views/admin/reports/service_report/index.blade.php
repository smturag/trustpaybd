    ...
    <div class="col-xl-12 mx-auto">
        ...
        <form method="get" id="search-form" class="mb-5" accept-charset="utf-8" role="form"
            ...
            <div class="row">
                ...
                <div class="col-md-2">
                    <div class="form-group">
                        <label for="selectMerchant">Select Sub Merchant</label>
                        <select class="form-control" name="selectSubMerchant" id="selectSubMerchant">
                            <option value="">Select Sub Merchant</option>
                            @foreach (App\Models\Merchant::where('merchant_type','sub_merchant')->get() as $merchant)
                                <option value="{{ $merchant->id }}" {{ request('selectMerchant') == $merchant->id ? 'selected' : '' }}>
                                    {{ $merchant->fullname }}
                                </option>
                            @endforeach
        ...
        @if(request()->route('service') == 'deposit')
            @include('admin.reports.service_report.deposit_service', ['results' => $results])
        @elseif(request()->route('service') == 'withdraw')
            @include('admin.reports.service_report.withdraw_service', ['results' => $results])
        @endif
...
<script>
    document.getElementById('search-form').addEventListener('submit', function(e) {
        ...
        this.action = `/admin/report/service/${selectedService}`;
    });
</script>
@endpush
